...
- `inbox list` flags `-u, --unread` and `-r, --read` to filter by status (mutually exclusive)
- MCP `list_inbox` params `unreadOnly` and `readOnly` mirroring the CLI filters

### Changed

- `inbox list` now returns the full inbox (read and unread) by default; status is conveyed per message via the `isUnread` flag rather than filtered out. Use `--unread` to restore the previous unread-only behavior.
